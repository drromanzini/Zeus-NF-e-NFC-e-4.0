...
namespace NFe.Danfe.Fast.NFCe
{
    ...
    public class DanfeFrNfce: DanfeBase
    {
        ...
        public DanfeFrNfce(nfeProc proc, ConfiguracaoDanfeNfce configuracaoDanfeNfce, string cIdToken, string csc)
        {
            ...
            //((TextObject)Relatorio.FindObject("txtUrl")).Text = proc.NFe.infNFeSupl.urlChave;//proc.NFe.infNFeSupl.ObterUrl(proc.NFe.infNFe.ide.tpAmb, proc.NFe.infNFe.ide.cUF, TipoUrlConsultaPublica.UrlConsulta);
            ((BarcodeObject)Relatorio.FindObject("bcoQrCode")).Text = proc.NFe.infNFeSupl.qrCode; //proc.NFe.infNFeSupl  == null ? proc.NFe.infNFeSupl.ObterUrlQrCode(proc.NFe, cIdToken, csc) : proc.NFe.infNFeSupl.qrCode;
            ((TextObject)Relatorio.FindObject("txtDescPagto")).Text = ((FormaPagamento)proc.NFe.infNFe.pag.detPag[0].tPag).Descricao();
            ((TextObject)Relatorio.FindObject("Text22")).Text = proc.NFe.infNFe.pag.detPag[0].vPag.ToString("n2");
